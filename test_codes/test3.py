...
with mp_selfie_segmentation.SelfieSegmentation(
    ...
    while True:
        ...
        mask = (results.segmentation_mask > 0.1).astype(np.uint8) * 255  # 轉換為二值化遮罩
        three_channel_mask = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)      # 轉換為三通道遮罩

        output_image = np.where(three_channel_mask == 255, img, bg)
