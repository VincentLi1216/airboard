...
from utils.conv_array import conv_array, diff_array, filter_np_array
...
def find_critical_indices(dir_path, save_path=None, to_show=False, show_plot=False):
    ...
    for i in tqdm(range(len(file_paths)-1)):
        ...
        if save_path != None:
            cv2.imwrite(
                f"./example_dir/subtrack_back_with_mask/{i}.png", result)
    ...
    if show_plot:
        ...
        plt.scatter(np.array(min_indices),
                    pixel_list[min_indices], color="c", marker="v")
