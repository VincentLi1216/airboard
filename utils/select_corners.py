...
def render(input_img, defualt_points=True):
    ...
    for i in range(len(points_list)):
        ...
        if i == len(points_list)-1:
            img = cv2.line(img, points_list[0],
                           points_list[i], (255, 255, 255), 1)
        # other lines
        else:
            img = cv2.line(img, points_list[i],
                           points_list[i+1], (255, 255, 255), 1)
            pass
...
def select_corner(input_img):
    ...
    cv2.setMouseCallback('Select ROI (Press q to quit)',
                         interaction)  # 設定偵測事件的函式與視窗
